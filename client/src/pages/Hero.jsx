...
const SaveinDb=async()=>{
  ...
  try {
    const res=await axios.post(url+"/api/img/save_image",{newimage:serverimg},{
      withCredentials:true
    })
    if(res.data.status){
      toast.success(res.data.message);
  ...
  Turn Your Imagination Into Stunning Images
        ...
        <div className='mb-3'>
            ...
            {serverimg&&<div>
              ...
              <div>
                <button className='text-cyan-800 bg-gray-400 p-4 rounded-lg hover:bg-gray-600 hover:text-purple-300 transition ease-in-out duration-200' onClick={DownloadIMage}><IoIosCloudDownload/></button>
              </div>
              </div>
